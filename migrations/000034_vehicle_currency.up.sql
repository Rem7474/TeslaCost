-- ============================================================================
-- TeslaCost Vehicle Currency Migration (Up)
-- Database: PostgreSQL 14+
-- ============================================================================

-- A vehicle's base currency, fixed at creation (existing vehicles keep 'EUR', matching their
-- actual history) and never editable afterward: costs are money.Cents integers mixed straight
-- into cost_ledger and other sums, so changing it later would mean retroactively converting
-- every stored amount at a single, necessarily arbitrary rate. fx_rate on an individual expense
-- keeps meaning "conversion rate to the vehicle's currency" (previously always the euro).
ALTER TABLE vehicles ADD COLUMN currency TEXT NOT NULL DEFAULT 'EUR' CHECK (currency ~ '^[A-Z]{3}$');

-- Re-create cost_ledger (last defined in 000023_ice_vehicles) so the three branches that
-- convert a foreign-currency amount compare against the vehicle's own currency instead of a
-- hard-coded euro. Every other branch is copied verbatim.
...
SELECT c.vehicle_id,
       ...
       c.id AS source_id,
       ROUND(CASE WHEN c.currency = v.currency THEN c.cost ELSE c.cost * c.fx_rate END, 2) AS amount_eur
FROM charge_logs c
JOIN vehicles v ON v.id = c.vehicle_id
WHERE c.deleted_upstream_at IS NULL
  AND c.cost IS NOT NULL
  AND (c.currency = v.currency OR c.fx_rate IS NOT NULL)

UNION ALL
-- Energy: fuel fill-ups (combustion vehicles)
SELECT f.vehicle_id,
       f.date AS entry_date,
       'ENERGY'::text,
       'fuel_logs'::text,
       f.id,
       ROUND(f.amount, 2)
...
SELECT e.vehicle_id,
       ...
       'drive_expenses',
       e.id,
       ROUND(CASE WHEN e.currency = v.currency THEN e.amount ELSE e.amount * e.fx_rate END, 2)
FROM drive_expenses e
JOIN vehicles v ON v.id = e.vehicle_id
WHERE e.currency = v.currency OR e.fx_rate IS NOT NULL
...
SELECT m.vehicle_id,
       ...
       m.id,
       ROUND(CASE WHEN m.currency = v.currency THEN m.amount ELSE m.amount * m.fx_rate END, 2)
...
WHERE m.currency = v.currency OR m.fx_rate IS NOT NULL
