...
import { RefreshCw, Car, Gauge, Plus } from 'lucide-vue-next'
...
<template>
  <header class="bg-slate-900/60 backdrop-blur-md border-b border-slate-800 sticky top-0 z-40 px-4 py-3 flex items-center justify-between">
    ...
    <div class="flex items-center gap-2">
      ...
      <span v-if="vehicleStore.syncResult" class="hidden md:inline text-[11px] text-emerald-400">
        +{{ vehicleStore.syncResult.drives_synced }} trajets, +{{ vehicleStore.syncResult.charges_synced }} charges
      </span>
    </div>
  </header>
</template>
